...
//                Log.d(TAG,"progress:"+progress);
    ...
    private void initAtts() {
        ...
        middleLineColor = typedArray.getColor(R.styleable.recordView_middleLineColor, Color.parseColor("#45C3E5"));
        voiceLineColor = typedArray.getColor(R.styleable.recordView_middleLineColor, Color.parseColor("#45C3E5"));
        middleLineHeight = typedArray.getDimension(R.styleable.recordView_middleLineHeight, 2);
        timeTextColor = typedArray.getColor(R.styleable.recordView_timeTextColor, Color.parseColor("#FF84AD"));
...
//        }
    ...
    private void drawDefaultForPlay(Canvas canvas) {
        ...
        mPaint.setColor(Color.parseColor("#DAF6FE"));
        ...
        paint2.setColor(Color.parseColor("#45C3E5"));
...
//        }
        ...
        paint2.setColor(Color.parseColor("#FF84AD"));
        paint2.setTextAlign(Paint.Align.CENTER);
        canvas.drawText(countdownTime2 + "", getWidth() / 2, getHeight() / 2 + 50, paint2);
...
//        canvas.drawText(unit == null ? "s" : unit, timeWidth, getHeight() / 2 - 20, paint1);
    ...
    private void drawProgress(Canvas canvas) {
         ...
         */
        if (progress > 90) {
            mPaint.setColor(Color.parseColor("#45C3E5"));
            mPaint.setStrokeWidth(dip2px(mContext, widthing));
            RectF oval = new RectF(dip2px(mContext, pandding)
                    , dip2px(mContext, pandding)
                    , getWidth() - dip2px(mContext, pandding)
        ...
        RectF oval2 = new RectF(dip2px(mContext, pandding)
                , dip2px(mContext, pandding)
                , getWidth() - dip2px(mContext, pandding)
                , getHeight() - dip2px(mContext, pandding));
        //看这里，其实这里当progress大于90以后就一直只画0-90度的圆环
        canvas.drawArc(oval2, 0, progress < 90 ? progress : 90, false, mPaint);    //绘制圆弧
        canvas.rotate(90, getWidth() / 2, getHeight() / 2);
        mPaint.reset();
    ...
    public void startCountDownView(final int times) {
        countdownTime2 = times;


        valueAnimator = ValueAnimator.ofInt(360);
        valueAnimator.setInterpolator(new LinearInterpolator());
        valueAnimator.setDuration(1000);
        valueAnimator.setRepeatCount(times - 1);
        ...
        valueAnimator2 = ValueAnimator.ofInt(times, 0);
        valueAnimator2.setDuration(times * 1000);
        valueAnimator2.setInterpolator(new LinearInterpolator());
        valueAnimator2.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                countdownTime2 = (int) valueAnimator.getAnimatedValue();
                postInvalidate();
            }
        });
...
//            float y = event.getY();
//            Log.d("点击位置", "x:" + x + "---y:" + y);
//            Log.d("点击位置", "pandding:" + dip2px(mContext, pandding) + "---getWidth:" + (getWidth() - dip2px(mContext, pandding)));
//            Log.d("点击位置", "pandding:" + dip2px(mContext, pandding) + "---getHeight:" + (getHeight() - dip2px(mContext, pandding)));
//            if (x > dip2px(mContext, pandding) && x < (getWidth() - dip2px(mContext, pandding))
//                    || y < dip2px(mContext, pandding) && y > (getHeight() - dip2px(mContext, pandding))) {
//                Log.d("点击位置", "开始");
//                start();
//                timeTimer2.schedule(timeTask2 = new TimerTask() {
//                    public void run() {
//                        Message msg = new Message();
//                        msg.what = 1;
...
//                    }
//                }, 20, 20);
//                setOnCountDownListener(new RecordView.OnCountDownListener() {
//                    @Override
//                    public void onCountDown() {
//                        Toast.makeText(mContext, "计时结束啦~~", Toast.LENGTH_SHORT).show();
//                    }
//                });
//            }
//            return true;
